...
def transcribe_canary(input_file, output_file, config):
    ...
    model_name = config.get("model_size", "nvidia/canary-1b")
    device = config.get("device", "cuda")
    if device == "cuda" and not torch.cuda.is_available():
        print("Warning: CUDA not available, falling back to CPU for Canary model.")
        device = "cpu"
    ...
    asr_model = nemo_asr.models.EncDecMultiTaskModel.from_pretrained(model_name)
    ...
    if not hypotheses or not hypotheses[0].word_alignments:
        print("Could not generate timestamps for Canary.")
